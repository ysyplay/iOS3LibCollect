...
//  Copyright (c) 2016 ideawu. All rights reserved.
//
...
- (void)viewDidLoad {
	...
	decWithPrivKey = [RSA decryptString:encWithPubKey privateKey:privkey];
	NSLog(@"服务端用公钥加密后 客户端用私钥解密: %@", decWithPrivKey);
	...
	NSLog(@"服务端用私钥加密后 客户端用公钥解密: %@", decWithPublicKey);
}
