...
static NSInteger const MAXYEAR = 2200;
static NSInteger const MINYEAR = 1900;
    ...
    if (self) {
        ...
        switch (datePickerStyle) {
            ...
            case DateStyleShowYearMonthDayHourMinute:
                ...
                break;
            case DateStyleShowYearMonthDayHourMinuteSeconds:
...
-(void)addLabelWithName:(NSArray *)nameArr
{
    ...
    for (int i=0; i<nameArr.count; i++)
    {
        ...
        if (i==0)
        {
            labelX = kPickerSize.width/(nameArr.count*2)+18;
        }
        else
        {
            labelX = kPickerSize.width/(nameArr.count*2)+13+kPickerSize.width/nameArr.count*i;
        }
...
-(NSArray *)getNumberOfRowsInComponent {
    ...
    NSInteger secondsNum = _secondsArray.count;
    ...
    switch (self.datePickerStyle) {
        ...
        case DateStyleShowYearMonthDayHourMinute:
            ...
            break;
        case DateStyleShowYearMonthDayHourMinuteSeconds:
            return @[@(yearNum),@(monthNum),@(dayNum),@(hourNum),@(minuteNUm),@(secondsNum)];
            break;
        case DateStyleShowMonthDayHourMinute:
...
- (void)getNowDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    if (self.datePickerStyle == DateStyleShowYearMonthDayHourMinuteSeconds)
        indexArray = @[@(yearIndex),@(monthIndex),@(dayIndex),@(hourIndex),@(minuteIndex),@(secondsIndex)];
